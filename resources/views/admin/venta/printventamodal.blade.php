...
<!-- Modal -->
<div class="modal fade" id="printVentaModal{{ $venta->id }}" tabindex="-1" aria-labelledby="printVentaModal{{ $venta->id }}" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="printVentaModal{{ $venta->id }}Title">
                    <i class="bi bi-printer text-info"></i> Imprimir
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ url('print-venta') }}" method="GET" target="_blank">
                <div class="modal-body">
                    ...
                    <div class="row gx-3 p-3">
                        ...
                        <div class="col-md-4 mb-3">
                            <!-- Form Field Start -->
                            <div class="mb-3">
                                <label for="pdftipo" class="form-label">Para</label>
                                <select name="pdftipo" class="form-select" aria-label="Default select example">
                                    <option value="Cliente "{{ request('pdftipo') == 'Cliente' ? ' selected' : '' }}>Cliente</option>
                                    <option value="Usuario"{{ request('pdftipo') == 'Usuario' ? ' selected' : '' }}>Usuario</option>
                                </select>
                            </div>
                        </div>
