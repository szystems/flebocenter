...
<!-- Modal -->
<div class="modal fade" id="printIngresosModal" tabindex="-1" aria-labelledby="printIngresosModal" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="printIngresosModalTitle">
                    <i class="bi bi-printer text-info"></i> Imprimir
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ url('print-ingresos') }}" method="GET" target="_blank">
                <div class="modal-body">
                    ...
                    <div class="row gx-3 p-3">
                        ...
                        <input type="hidden" name="desde_imprimir" value="{{ $fechaDesdeVista }}">
                        <input type="hidden" name="hasta_imprimir" value="{{ $fechaHastaVista }}">
                        <input type="hidden" name="proveedor_imprimir" value="{{ request('proveedor_id') }}">
                        <input type="hidden" name="tipocomprobante_imprimir" value="{{ request('tipo_comprobante') }}">
                        <input type="hidden" name="numerocomprobante_imprimir" value="{{ request('numero_comprobante') }}">
                        <input type="hidden" name="saldo_imprimir" value="{{ request('saldo') }}">
